...
import { InfoCircleOutlined, RadarChartOutlined, TranslationOutlined } from '@ant-design/icons'
import './footer.styl'
...
export default auto(function FooterEntry (props) {
  ...
  function renderOpsIntelligence () {
    const openCount = props.store.opsInsights.filter(item => item.status === 'open').length
    return (
      <div className='terminal-footer-unit terminal-footer-ops'>
        <button
          className='ops-footer-entry'
          onClick={() => window.store.openOpsPanel()}
          title='TermMind 智能运维'
        >
          <RadarChartOutlined />
          <span>智维</span>
          {openCount > 0 && <i>{openCount > 9 ? '9+' : openCount}</i>}
        </button>
      </div>
    )
  }
