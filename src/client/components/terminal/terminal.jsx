...
} from '../../common/ops-intelligence.js'
...
class Term extends Component {
  constructor (props) {
    ...
    this.shellType = null
    this.opsActiveCommand = null
    this.opsOutputBuffer = ''
  }
  ...
  componentWillUnmount () {
    if (this.opsActiveCommand) {
      this.finishOpsCommandCapture(null, true)
    }
    refs.remove(this.id)
  ...
  analyzeWithTermMind = () => {
    window.store.analyzeOpsSelection(
      this.term.getSelection(),
      this.props.tab
    )
  }
  ...
  saveAsOpsKnowledge = () => {
    window.store.captureOpsKnowledgeFromSelection(
      this.term.getSelection(),
      this.props.tab
    )
  }
  ...
  renderContextMenu = () => {
    ...
    const items = [
      ...
      {
        key: 'analyzeWithTermMind',
        icon: <AIIcon />,
        label: '使用 TermMind 智能诊断',
        disabled: !hasSelection
      },
      {
        key: 'saveAsOpsKnowledge',
  ...
  handleInputEvent = (d) => {
    ...
    if (d === '\r' || d === '\n') {
      ...
      if (currentCmd && currentCmd.trim() && this.shouldUseManualHistory()) {
        ...
        this.startOpsCommandCapture(currentCmd.trim())
      }
  ...
  /**
   * Capture command/output pairs for the local-first operations timeline.
   * Shell integration gives reliable command boundaries while the attach
   * addon forwards server output only (never local password keystrokes).
   */
  startOpsCommandCapture = (command) => {
    if (this.opsActiveCommand) {
      this.finishOpsCommandCapture(null, true)
    }
    const tab = this.props.tab || {}
    this.opsOutputBuffer = ''
    this.opsActiveCommand = {
      command,
      cwd: this.cmdAddon?.getCwd?.() || tab.path || '',
      startedAt: Date.now(),
      tabId: tab.id,
      host: tab.title || tab.host || (this.isLocal() ? 'local' : 'unknown'),
      address: tab.host || '',
      username: tab.username || '',
      connectionType: tab.type || 'ssh'
    }
  }

  onOpsTerminalOutput = (value) => {
    if (!this.opsActiveCommand || !value) {
      return
    }
    this.opsOutputBuffer += value
    // Keep a bounded rolling window so a noisy command cannot grow renderer
    // memory indefinitely. The final sanitizer applies a smaller DB limit.
    if (this.opsOutputBuffer.length > 48000) {
      this.opsOutputBuffer = this.opsOutputBuffer.slice(-48000)
    }
    if (!this.cmdAddon?.hasShellIntegration?.()) {
      clearTimeout(this.timers.opsManualCapture)
      this.timers.opsManualCapture = setTimeout(() => {
        this.finishOpsCommandCapture(null)
      }, 1800)
    }
  }

  finishOpsCommandCapture = (exitCode, interrupted = false) => {
    if (!this.opsActiveCommand) {
      return
    }
    ...
    const event = {
      ...this.opsActiveCommand,
      output: sanitizeTerminalOutput(this.opsOutputBuffer),
      exitCode,
      interrupted,
      endedAt,
      durationMs: Math.max(0, endedAt - this.opsActiveCommand.startedAt)
    }
    this.opsActiveCommand = null
    this.opsOutputBuffer = ''
    window.store.recordOpsCommand(event)
  }
  ...
  initTerminal = async () => {
    ...
    this.cmdAddon.onCommandExecuted((cmd) => {
      if (cmd && cmd.trim()) {
        ...
        this.startOpsCommandCapture(cmd.trim())
      }
    })
    this.cmdAddon.onCommandFinished((exitCode) => {
      this.finishOpsCommandCapture(exitCode)
    })
